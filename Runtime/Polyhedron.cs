...
namespace EyE.Geometry
{
    ...
    public class Face
    {
        ...
        {
            faceOf = p;
            RebuildCornerListFromCornerIndices();
        }
        [System.NonSerialized]
        public List<Corner> corners;
        [SerializeField]
        private List<int> cornerIndexes;
        private void RebuildCornerIndicesFromCornerList()
        {
            cornerIndexes = new List<int>(corners.Count);
            for (int i = 0; i < corners.Count; i++)
            {
                int idx = faceOf.corners.IndexOf(corners[i]);
                if (idx == -1)
                    throw new GeometryException("Corner not found in polyhedron corner list.");
                cornerIndexes.Add(idx);
            }
        }
        private void RebuildCornerListFromCornerIndices()
        {
            if (faceOf == null || cornerIndexes == null)
                throw new GeometryException("Face is missing reference to polyhedron or corner indexes");

            corners = new List<Corner>(cornerIndexes.Count);
            for (int i = 0; i < cornerIndexes.Count; i++)
            {
                int idx = cornerIndexes[i];
                if (idx < 0 || idx >= faceOf.corners.Count)
                    throw new GeometryException($"Corner index {idx} is out of range for polyhedron corner list.");
                corners.Add(faceOf.corners[idx]);
            }
        }
        /// <summary>
        ...
        public Face(Polyhedron faceOf, List<Corner> corners)
        {
            ...
            RebuildCornerIndicesFromCornerList();
            if (corners.Count < 3)
        ...
        {
            if (corners != null && faceOf != null)
                RebuildCornerIndicesFromCornerList();
        }
        public void OnAfterDeserialize()
        {
            //post deserialization operations completed in SetPoly function, which the polyhedron class will call after deserialization.
        }
        public override string ToString()
        {
    ...
    public class Edge
    {
        ...
        {
            edgeOfPolyhedron = p;
            endpointA = p.corners[endpointACornerIndex];
            endpointB = p.corners[endpointBCornerIndex];
        }
        [System.NonSerialized]
        public Corner endpointA;
        public int endpointACornerIndex;
        [System.NonSerialized]
        public Corner endpointB;
        public int endpointBCornerIndex;
        ...
        public Edge(Polyhedron edgeOf, Corner endpointA, Corner endpointB)
        {
            ...
            PopulateCornerIndicesFromEndpoints();
        }

        private void PopulateCornerIndicesFromEndpoints()
        {
            endpointACornerIndex = edgeOfPolyhedron.corners.IndexOf(endpointA);
            endpointBCornerIndex = edgeOfPolyhedron.corners.IndexOf(endpointB);
        }
    ...
    public class Polyhedron : ISerializationCallbackReceiver
    {
        ...
        {
            MeshData meshData = new MeshData(fromMesh);
            PopulateFromMesh(meshData, null, true).AsTask().GetAwaiter().GetResult();
            return;
            /*
        ...
        static public async UniTask<Polyhedron> CreateFromMesh(MeshData fromMesh, CancelBoolRef cancelRef, bool runSyncronosly = false)
        {
            ...
            await poly.PopulateFromMesh(fromMesh, cancelRef, false);
            return poly;
        }

        public async UniTask PopulateFromMesh(MeshData fromMesh, CancelBoolRef cancelRef, bool runSynchronosly = false)
        {
            YieldTimer yieldTimer = new YieldTimer(cancelRef, runSynchronosly);
            //loop through mesh triangles and vertex array to populate the coner face and edge lists- start with corners
            Dictionary<Vector3, Corner> cornerByPosition = new Dictionary<Vector3, Corner>();
            int[] vertIndexToCornerIndex = new int[fromMesh.vertexCount]; // provide mapping from vertex index into a corner index (in the corners list- only valid until corners are removed from this list, later on)
            corners = new List<Corner>();
            ...
            await RecomputeEdgesAsync(cancelRef, runSynchronosly);
            ...
            bool FacesArePlanar(Face faceA, Face faceB)
            {
                return faceA.normal == faceB.normal;
                //  Debug.Log("..checking normals: " + faceA.normal + " , " + faceB.normal +" :  "+ Vector3.Dot(faceA.normal, faceB.normal));
                //return Mathf.Abs(Vector3.Dot(faceA.normal, faceB.normal)) > .99f;
            }
            //merge faces
        ...
        /// </summary>
        async UniTask RecomputeEdgesAsync(CancelBoolRef cancelRef, bool runSychronously = false)
        {

            YieldTimer yieldTimer = new YieldTimer(cancelRef, runSychronously);
            edges.Clear();
            foreach (Face eachFace in faces)
            {
                ...
                // end debug stuff
                */
                if (!runSychronously) await yieldTimer.YieldOnTimeSlice();
        ...
        /// </summary>
        public async UniTask<Polyhedron> SmoothAsync(CancelBoolRef cancelRef)
        {
            Polyhedron smoothedPoly = new Polyhedron(this);
            Dictionary<Corner, Vector3> newVertPerCornder = new Dictionary<Corner, Vector3>();
            YieldTimer yieldTimer = new YieldTimer(cancelRef);
            foreach (Corner c in smoothedPoly.corners)
            {
        ...
        public async UniTask<Polyhedron> DualAsync(CancelBoolRef cancelRef)
        {
            ...
            foreach (Face oldFace in this.faces)//these faces will become corners in the new poly
            {
                ...
                newCorners.Add(new Corner(dualPoly, center));//part of dual operation, where face becomes corner
                // Yield if the time slice has been exceeded
                await yieldTimer.YieldOnTimeSlice();
            }

            foreach (Corner oldCorner in this.corners)//these corners will become faces in the new poly
            {
                List<Corner> newFaceCorners = new List<Corner>();// this list will be populated with references to the corners we (above) put into newCorners list
                List<Face> oldCornersOldFaces = oldCorner.touchingFaces; //gets a list of faces that the original corner touched.
                //  Debug.Log("old corner at index: " + corners.IndexOf(oldCorner) + " touching " + oldCornersOldFaces.Count + "faces");

                foreach (Face oldFaceTouchingOldCorner in oldCornersOldFaces)//loop through the faces the corner touches
                {
                    int newCornerIndex = this.faces.IndexOf(oldFaceTouchingOldCorner);// the NEW corner index will be the same as the OLD face index
                    newFaceCorners.Add(newCorners[newCornerIndex]);// get the referenced corner, and add it to this face.
                }
                newFaces.Add(new Face(dualPoly, newFaceCorners));//instantiate and add a new face for the old corner.
                // Yield if the time slice has been exceeded
                await yieldTimer.YieldOnTimeSlice();
            }
            foreach (Face f in newFaces)
            ...
            await dualPoly.RecomputeEdgesAsync(cancelRef);//edges are recomputed, populating the edgeList of the new polyhedron
        ...
        public bool CheckIntegrity(Transform debugDragAt, Gradient orderGradient)
        {
            ...
            foreach (Corner c in corners)
            {
                ...
                {
                    Debug.Log("Found corner ["+cornerCounter+"] touching no faces: invalid");
                    return false;
                }
                if (c.touchingFaces.Count < 3)
                {
                    Debug.Log("Found corner [" + cornerCounter + "] touching less than 3 faces- valid for 2d objects only");
                }

                if (c.touchingEdges.Count < 2)
                {
                    Debug.Log("Found corner [" + cornerCounter + "] touching one or no edges: invalid");
                    return false;
                }
                else
                {
                    ...
                    {
                        Debug.Log("Found corner [" + cornerCounter + "] touching less than 3 edges- valid for 2d objects only");
                    }
    ...
    1, 3, 2   // Face 4
        ...
        public (Polyhedron, Mesh) Icosphere(int levels)
        {
            // for each triangle
            //      add to processList
            // for each level
            ...
            for (int level = 0; level < levels; level++)
            {
                while (trisToProcess.Count > 0)
                {
                    ...
                    if (!vertexToCornerIndex.ContainsKey(Bindex))
                        vertexToCornerIndex.Add(Bindex, corners.GetIndexOrAdd(B));
                    if (!vertexToCornerIndex.ContainsKey(Cindex))
